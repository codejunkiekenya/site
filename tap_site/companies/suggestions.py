...
def update_clusters():
    ...
    if num_reviews % update_step == 0: # using some magic numbers here, sorry...
        ...
        all_companies_ids = set(map(lambda x: x.companies.id, Review.objects.only("companies")))
        num_users = len(all_user_names)
        ratings_m = dok_matrix((num_users, max(all_companies_ids)+1), dtype=np.float32)
        for i in range(num_users): # each user corresponds to a row, in the order of all_user_names
            ...
            for user_review in user_reviews:
                ratings_m[i,user_review.companies.id] = user_review.rating
