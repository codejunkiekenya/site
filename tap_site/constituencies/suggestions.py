...
def update_clusters():
    ...
    if num_reviews % update_step == 0: # using some magic numbers here, sorry...
        ...
        all_constituencies_ids = set(map(lambda x: x.constituencies.id, Review.objects.only("constituencies")))
        num_users = len(all_user_names)
        ratings_m = dok_matrix((num_users, max(all_constituencies_ids)+1), dtype=np.float32)
        for i in range(num_users): # each user corresponds to a row, in the order of all_user_names
            ...
            for user_review in user_reviews:
                ratings_m[i,user_review.constituencies.id] = user_review.rating
